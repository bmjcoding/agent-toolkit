...
The `/improve accept` subcommand writes a new entry on each accepted fix. The `~/.claude/metadata/` directory receives its own `CHANGELOG.md` because the schema is versioned and future migrations need a history.
...
`claude-code/skills/retro/scripts/meta-retro-batch.py` aggregates the most recent N (default 10) retro JSON files from `~/.claude/retros/orchestrator/`. It reports five signal categories: recurring unresolved findings, severity-category skew, low-signal P2 noise, `frankenstein_lines` size trajectory, and cross-session recommendation persistence. The script uses only Python stdlib.
...
**P4**: The four-bullet untrusted-data prelude and instruction sandwich is extracted to a shared reference file at `claude-code/skills/improve/references/security-preamble.md`. Each of the 11 agents that previously contained the verbatim block now includes a pointer to that file instead. This factoring reduces the corpus by approximately 200 lines.
...
- Cross-session pattern detection is now possible once 10+ retros accumulate in `~/.claude/retros/orchestrator/`.
...
- Each of the 11 agents that adopted the shared preamble now has a sub-file dependency on `claude-code/skills/improve/references/security-preamble.md`. If that file is moved or renamed, all 11 agents need updating.
...
- Shared preamble: `claude-code/skills/improve/references/security-preamble.md`
- Meta-retro script: `claude-code/skills/retro/scripts/meta-retro-batch.py`
