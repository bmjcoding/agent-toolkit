...
- Every footer compare link resolves to a valid URL once historical tags are created via `scripts/backfill-changelog-tags.sh`.
...
- `scripts/detect-platform.sh` depends on `git remote` being configured. New repositories without a configured origin fall through to `unknown` and receive the plain-text fallback.
