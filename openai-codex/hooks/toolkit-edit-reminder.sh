#!/usr/bin/env bash
# Ported from claude-code/hooks/toolkit-edit-reminder/toolkit-edit-reminder.sh for Codex CLI hooks (experimental)
...
# Codex mapping: PreToolUse / matcher: Edit|Write
# Note: stdin JSON field names (.tool_name, .tool_input.file_path, .tool_input.path) are
# Claude Code conventions. Verify Codex uses the same field names and update jq expressions
...
echo "$FILE" | grep -qE '(agent-toolkit|claude-toolkit|/\.claude)/(agents|skills|hooks|commands|rules|claude-code|shared)/[^/]+/[^/]+|^claude-code/(agents|hooks|commands)/[^/]+/[^/]+|^(skills|rules)/[^/]+/[^/]+' || exit 0
