#!/usr/bin/env bash
# Context injection hook for the Codex CLI surface (experimental)
...
- **Single-Writer Rule**: Before modifying any source file, run: `ls -t ${HANDOFF_DIR}/*.json 2>/dev/null | head -10`. If another agent's handoff lists the same file in `files_written`, do NOT modify it — report the conflict instead.
- **Handoff Protocol**: Your structured results MUST appear in your final message as a fenced ```handoff JSON block. A SubagentStop hook writes it to `${HANDOFF_DIR}/`. Do not write handoff files yourself.
RULES
)
...
if [[ -f "$BRIEF" ]]; then
  jq -n --arg ctx "$CONSTRAINTS" '{
    hookSpecificOutput: {
      additionalContext: ("Orchestrator context available. Read '"$BRIEF"' for project overview and '"$ORCH_BASE"'/plan.json for task decomposition.\n\n" + $ctx)
    }
  }'
else
