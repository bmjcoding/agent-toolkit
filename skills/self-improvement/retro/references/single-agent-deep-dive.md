...
**Rewrite signal**: If 3+ findings in the retro trace back to the same skill's instructions (vague steps, missing coverage, conflicting guidance), the skill may need a structural rewrite rather than individual patches. Recommend running `definition-review` on it to get a full quality assessment.
