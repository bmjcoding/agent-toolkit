...
**Rewrite threshold**: If 5+ findings target the same skill or agent definition, run an inline definition-review check first (linter + semantic review). If the verdict is **REWRITE**, stop and recommend the user run `definition-review` on it. Do not attempt to patch a fundamentally broken definition. If the verdict is **NEEDS WORK** or **PASS**, apply the findings normally.
  - *P0 carve-out*: If a REWRITE verdict is returned but any of the findings is P0, apply that P0 finding only, then surface the recommendation to run `definition-review` for the remaining findings. A P0 blocker must not be left unresolved. After applying the P0 fix, still run step 5 (final quality gate) on the modified file before reporting.
...
- **Skills**: `skills/{category}/{name}/CHANGELOG.md` (or legacy `skills/{name}/CHANGELOG.md` during migration)
...
- **Reference files and scripts inside a component directory** (e.g., `skills/{category}/{name}/references/`, `agents/{name}/evals/`): append to that component's `CHANGELOG.md`, not a global one
...
2. Under `## [Unreleased]`, find or create the appropriate category header (`### Added`, `### Changed`, `### Fixed`, etc. — choose based on the change type; canonical order per the `changelog` skill).
3. Append a verb-prefixed one-liner bullet (`- `) under that category header.

For entry content format and category rules, see the `changelog` skill as the authoritative spec (Keep a Changelog 1.1.0).

If the changelog file doesn't exist, create it with the required header block from the `changelog` skill, then add `## [Unreleased]` and the entry.
