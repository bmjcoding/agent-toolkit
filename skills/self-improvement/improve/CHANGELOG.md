...
- Updated validation, retro-history, and changelog routing instructions to find
  skills in categorized directories while keeping legacy flat-path fallbacks.
- Renamed definition quality-gate references and linter discovery fallbacks to
  `definition-review`.
...
- Replaced the `remove` subcommand `argument-hint` angle-bracket placeholder with a
  bracket placeholder so the canonical definition passes deterministic `definition-review`
  lint in CI.
...
- Updated shared wording to refer to `retro`, `definition-review`, and `improve` as cross-tool workflows rather than Claude-specific slash-command forms.
...
- `argument-hint` now uses bracket placeholders (`[rec-id]`) instead of angle brackets so
  the canonical definition passes the deterministic `definition-review` structural lint used in CI.
...
- Replaced the `argument-hint` angle-bracket placeholder for the remove subcommand with a
  bracket placeholder so the frontmatter no longer trips `definition-review` lint rule `S06`.
