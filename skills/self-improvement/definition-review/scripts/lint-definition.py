...
def detect_type(filepath):
    ...
    if normalized.endswith("SKILL.md") or "skills" in parts:
        return "skill"
    if normalized.endswith("AGENT.md") or "agents" in parts:
        return "agent"
    return "unknown"
...
def lint_file(filepath, file_type=None, base=None):
    ...
    # S08: Referenced files exist. Skill-local references resolve from the skill
    # directory; agent references to repo helper scripts resolve from the repo root.
    ...
    for match in re.finditer(r"`((?:references|scripts|assets)/[^`]+)`", content):
        ...
        if file_type == "agent" and ref.startswith("scripts/"):
            base_dir = repo_root
    ...
    if file_type == "skill" and body_line_count > 300:
        refs_dir = os.path.join(definition_dir, "references")
    ...
    has_error_section = bool(re.search(r"(?i)(gotcha|troubleshoot|error|failure mode|common mistake|pitfall|caveat|watch out)", body))
    if not has_error_section:
        warn("Q07", "No gotchas, troubleshooting, or error handling section")

    # Q08: Agent missing tools
    if file_type == "agent":
        has_runtime_tools = "tools" in fm or "disallowedTools" in fm
        has_canonical_capabilities = "capabilities" in fm
        if not has_runtime_tools and not has_canonical_capabilities:
            warn("Q08", "Agent definition missing 'tools' or 'disallowedTools' — tool access is uncontrolled")

    # Q09: Agent missing maxTurns
    if file_type == "agent":
        has_runtime_turn_limit = "maxTurns" in fm
        has_generated_runtime_adapter = "model-tier" in fm and "adapters" in fm
        if not has_runtime_turn_limit and not has_generated_runtime_adapter:
    ...
    if always_patterns and never_patterns:
        ...
        conflicts -= {"include", "specify", "commit", "add", "create", "write", "read", "check", "set", "call", "pass", "push", "pull", "modify", "change", "update", "delete", "remove", "emit"}
        if conflicts:
