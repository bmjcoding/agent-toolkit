...
name: on-demand
description: >
  Use when `autoresearch-analyst` receives an ad-hoc skill or agent review
  request. Resolves targets, extracts user concerns, runs definition-review, and
  conditionally drives improve.
lifecycle: stable
---
...
- "the `<name>` skill" → find `skills/*/<name>/SKILL.md` (legacy fallback: `skills/<name>/SKILL.md`)
...
For each target, run the full definition-review workflow inline (linter + semantic review +
...
- **Don't fabricate concerns**: if the user says only "review the X skill" with no
  specifics, run definition-review and let the findings speak. Don't invent concerns.
- **User concerns are P1, not gospel**: definition-review findings take precedence on
...
- **Inline review**: use preloaded definition-review knowledge. Do NOT spawn child agents
