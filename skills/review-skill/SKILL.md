...
1. Discover all definition files in the directory:
   ```bash
   find $ARGUMENTS -name "SKILL.md" -o -name "*.md" | grep -E "(SKILL\.md|agents/[^/]+\.md)" | sort
   ```
...
3. If **2-3 files found** → process each sequentially through steps 1-3 below. Present individual verdicts and a combined summary table at the end. Use the same `## Batch Review Summary` table format as the 4+ case:
   ...
   | 2 | agents/planner.md | NEEDS WORK | 0 errors | 3 warnings | 2 |
   ```
...
5. After all reviews complete (parallel or sequential), produce a **Consolidated Required Changes** table that merges all NEEDS WORK findings across files into a single `/improve`-compatible table:
   ...
   |---|---|---|---|---|---|
   | 1 | Add output template | skills/retro/SKILL.md | No example output shown | P1 | fix |
   | 2 | Scope disallowedTools | agents/planner.md | Agent can Write but shouldn't | P1 | fix |
   | 3 | Add gotchas section | agents/planner.md | No error handling guidance | P2 | fix |
   ```
...
# fallback: python3 ~/.claude/skills/review-skill/scripts/lint-definition.py $ARGUMENTS --format json
```
...
2. **Re-review**: Run `/review-skill` on the rewritten definition.
3. **Evaluate**:
   - **PASS** → done, exit loop.
