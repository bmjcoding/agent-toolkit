...
- **Download-trigger verification for command-shape changes**: When a subtask replaces the install command generation pattern in a consumer dialog (e.g., switching from curl-embedded commands to unzip-from-Downloads commands via a shared helper), the `completion_criteria` MUST include an explicit download-trigger check: "Verify `window.open(archiveUrl)` is called in `handleDownload` before `setPhase` — or document why this dialog does not require a prior download trigger." Add the mechanical verification step: "Run `grep -n 'window.open' <file>` after edits to confirm the download trigger is present in `handleDownload`." Changing the command shape changes the consumer's responsibility for triggering downloads; the planner must propagate this trigger contract into completion criteria whenever a command-shape change is planned.
