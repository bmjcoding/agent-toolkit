...
- **Validate dispatch prompts**: the `dispatch-validate` PreToolUse hook enforces
  ...
  `python3 scripts/orchestrator/dispatch-validator.py` from the toolkit checkout for
  ad-hoc verification.
- **Improve batch sizing**: before dispatching improve, run
  `python3 scripts/orchestrator/dispatch-budget.py --files <N> --max-turns <maxTurns>`.
...
- **Multi-domain finding consolidation**: before dispatching improve with findings
  ...
  `python3 scripts/orchestrator/consolidate-findings.py --output consolidated-findings.json <audit1.json> ...`.
