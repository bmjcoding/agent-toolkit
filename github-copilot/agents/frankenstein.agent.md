...
Resolve the toolkit helper root once before invoking helper scripts:
`TOOLKIT_DIR="${AGENT_TOOLKIT_DIR:-${TOOLKIT_PATH:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}}"`.
All orchestrator helper scripts below live under `$TOOLKIT_DIR/scripts/orchestrator/`;
the routing config is `$TOOLKIT_DIR/scripts/orchestrator/routing-config.json`.
...
```bash
"$TOOLKIT_DIR/scripts/orchestrator/preflight-check.sh" "$SESSION_ID"
```
...
`$TOOLKIT_DIR/scripts/orchestrator/routing-config.json` rather than inlined here. Reading the config from disk
...
**HEAD-SHA drift check** (run before each group 2+): Run `$TOOLKIT_DIR/scripts/orchestrator/drift-check.sh`. The script reads `.orchestrator/session-base-sha`, compares to current HEAD, and exits non-zero on drift. **STOP** if drift is detected and offer the user: (1) rebase onto new HEAD and re-verify prior group output, or (2) abort pipeline.
...
```bash
"$TOOLKIT_DIR/scripts/orchestrator/group-structural-check.sh" "$SID" <group-N>
```
...
```bash
"$TOOLKIT_DIR/scripts/orchestrator/check-diff-budget.sh" "$SUBTASK_ID" "$SESSION_ID"
```
...
2. **Seed backlog**: Run `python3 "$TOOLKIT_DIR/scripts/orchestrator/seed-backlog.py" --session "$SID"`. Exit 1 means no findings to seed (informational); exit 2 is fatal.
...
**5a HEAD-SHA drift check** (run before spawning doc-writer): Run `$TOOLKIT_DIR/scripts/orchestrator/drift-check.sh`. **STOP** on non-zero exit. A collision detected at this stage means all Phase 2-4 edits may have been reverted on disk and need to be re-applied via the Resume Protocol.
