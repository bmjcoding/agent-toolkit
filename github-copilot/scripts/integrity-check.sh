...
# Collects github-copilot toolkit component files that should not change
...
collect_files() {
  ...
  if [ -d "${TOOL_DIR}/agents" ]; then
    while IFS= read -r -d '' f; do
      files+=("$f")
    done < <(find "${TOOL_DIR}/agents" -maxdepth 2 -name "*.md" ! -name "CHANGELOG.md" -type f -print0 2>/dev/null | sort -z)
  fi

  # Instruction files
  if [ -d "${TOOL_DIR}/instructions" ]; then
    while IFS= read -r -d '' f; do
      files+=("$f")
    done < <(find "${TOOL_DIR}/instructions" -maxdepth 1 -name "*.md" -type f -print0 2>/dev/null | sort -z)
  fi

  # Prompt files
  if [ -d "${TOOL_DIR}/prompts" ]; then
    while IFS= read -r -d '' f; do
      files+=("$f")
    done < <(find "${TOOL_DIR}/prompts" -maxdepth 2 -name "*.md" ! -name "CHANGELOG.md" -type f -print0 2>/dev/null | sort -z)
  fi
...
cmd_baseline() {
  ...
  {
    printf '# GitHub Copilot toolkit integrity baseline\n'
...
cmd_verify() {
  ...
  header "Verifying github-copilot toolkit integrity..."
  ...
  if [ "$mismatches" -eq 0 ]; then
    printf "\n${GREEN}All github-copilot toolkit files are clean.${RESET}\n"
    return 0
  else
...
cmd_warn() {
  ...
  if [ "$exit_code" -ne 0 ]; then
    printf "${YELLOW}[integrity-check] WARNING: GitHub Copilot toolkit file changes detected:${RESET}\n" >&2
...
case "$MODE" in
  ...
  *)
    ...
    printf "  ${BOLD}baseline${RESET}  — Hash all github-copilot toolkit files and write integrity baseline\n"
    ...
    printf "  alias copilot-baseline='%s baseline'\n" "$0"
    printf "  alias copilot-verify='%s verify'\n" "$0"
    exit 1
    ;;
esac
