...
- Replaced the inline ADR-numbering shell snippet with a reference to `~/.claude/scripts/next-adr-number.sh`, which uses `flock` for concurrency-safe number allocation.
