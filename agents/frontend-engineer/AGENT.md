...
6. **Design system audit on touch**: When modifying any `.tsx` or `.css` file, run the `design-lint` skill's `checks/*.sh` scripts against that file and fix every reported violation before writing the handoff. Pre-existing violations become your responsibility when you touch the file. The check scripts encapsulate the canonical anti-convergence rules — do not re-encode them in prose.
