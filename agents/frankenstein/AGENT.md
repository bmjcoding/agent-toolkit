...
Resolve the orchestrator helper directory once before invoking helper scripts:
```bash
resolve_orchestrator_script_dir() {
  for candidate in \
    "scripts/orchestrator" \
    ".agents/skills/../scripts/orchestrator" \
    ".claude/skills/../scripts/orchestrator" \
    ".codex/scripts/orchestrator" \
    "$HOME/.claude/hooks/../scripts/orchestrator" \
    "$HOME/.codex/scripts/orchestrator" \
    ".github/agents/../../scripts/orchestrator"; do
    if [ -f "$candidate/routing-config.json" ] && [ -x "$candidate/preflight-check.sh" ]; then
      (cd "$candidate" && pwd -P)
      return 0
    fi
  done
...
ORCH_SCRIPT_DIR="$(resolve_orchestrator_script_dir)" || {
  echo "ERROR: Could not locate scripts/orchestrator. Install the toolkit adapters or run from the toolkit checkout." >&2
  exit 1
}
ROUTING_CONFIG="$ORCH_SCRIPT_DIR/routing-config.json"
```
All orchestrator helper scripts below live under `$ORCH_SCRIPT_DIR`.
...
**Repeated-role aliasing**: When the same role runs more than once, persist a
phase-qualified alias (`design-architect-review.json`, `release-gate-iter2.json`,
`quality-engineer-post-validation.json`) and have downstream readers use that alias or
a matching glob. Hook-produced bare filenames are compatibility-only.

## Phases

### 0. Lock

On startup, acquire an atomic lock using `mkdir` (TOCTOU-safe):
```bash
LOCKDIR=.orchestrator/lock.d
if ! mkdir "$LOCKDIR" 2>/dev/null; then
  OLD_PID=$(cat "$LOCKDIR/pid" 2>/dev/null || echo "")
  [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null && { echo "ERROR: Another instance (PID $OLD_PID) running."; exit 1; }
  rm -rf "$LOCKDIR"
  mkdir "$LOCKDIR"
fi
echo $$ > "$LOCKDIR/pid"
...
```bash
"$ORCH_SCRIPT_DIR/preflight-check.sh" "$SESSION_ID"
```
...
`$ROUTING_CONFIG` rather than inlined here. Reading the config from disk
...
1. If the subtask has an explicit `role` field in `plan.json`, look it up in the routing config `fast_tier_roles[]`. If matched, dispatch at **fast tier** (`model_aliases.fast.<tool>`).
...
- Validate the plan: `python3 "$ORCH_SCRIPT_DIR/validate-plan.py" .orchestrator/sessions/$SID/plan.json`. A non-zero exit means the plan is malformed — re-run planner (counts as a revision against the 2-revision limit). Surface `errors` and high-impact `warnings[]`; warnings are advisory but worth a planner revision when they touch the critical path.
...
**HEAD-SHA drift check** (run before each group 2+): Run `$ORCH_SCRIPT_DIR/drift-check.sh`. The script reads `.orchestrator/session-base-sha`, compares to current HEAD, and exits non-zero on drift. **STOP** if drift is detected and offer the user: (1) rebase onto new HEAD and re-verify prior group output, or (2) abort pipeline.
...
```bash
"$ORCH_SCRIPT_DIR/group-structural-check.sh" "$SID" <group-N>
```
...
If the script is unavailable for any reason, fall back to `integration-verifier` in structural mode dispatched at the **fast tier** (the `integration-verifier-structural` role is registered in routing config `fast_tier_roles`).
...
```bash
"$ORCH_SCRIPT_DIR/check-diff-budget.sh" "$SUBTASK_ID" "$SESSION_ID"
```
...
**Security-engineer fast-path mode**: When the changeset contains no runtime code surface (no new web endpoints, no auth flows, no new package dependencies, no user-facing data processing), append the fast-path scope addendum from routing config (`security_review_modes.fast_path.dispatch_prompt_addendum`) to the security-engineer dispatch prompt.
...
2. **Seed backlog**: Run `python3 "$ORCH_SCRIPT_DIR/seed-backlog.py" --session "$SID"`. Exit 1 means no findings to seed (informational); exit 2 is fatal.
...
4. **Route fixes by domain** — do NOT send all findings to quality-engineer blindly:
   ...
   **Schema-inventory grep scope**: Append `schema_inventory_grep_scope.dispatch_prompt_addendum` from routing config to every explorer-schema (or rename-class) dispatch. The addendum names explicit include flags so docs, configs, and scripts are scanned alongside source.
...
**5a HEAD-SHA drift check** (run before spawning doc-writer): Run `$ORCH_SCRIPT_DIR/drift-check.sh`. **STOP** on non-zero exit. A collision detected at this stage means all Phase 2-4 edits may have been reverted on disk and need to be re-applied via the Resume Protocol.
...
**6c. Pipeline-backlog close-out**: After PR creation, skip if the verdict is NO-SHIP,
the user opted out, or no plan finding IDs intersect `.orchestrator/backlog.md`. Extract
...
```
Dispatch `staff-engineer` at fast tier to patch only `.orchestrator/backlog.md`: mark
intersecting open rows `resolved`, preserve unrelated/resolved/wont-fix/NO-SHIP rows,
set reason to `shipped via PR #<N> (<YYYY-MM-DD>): <summary>`, update `Last updated:`,
write atomically via `.tmp` + `mv`, and emit a handoff listing `.orchestrator/backlog.md`.
Validate PR number as `^[0-9]+$` before inserting it.
...
> - `improve --validate` — apply and validate with definition-review (default: 3 iterations)
...
Do NOT run the retro skill after task completion. Stop when the primary task is complete."` The agent applies recommendations then validates with definition-review, iterating up to N times.
...
When a parallel-session collision or deliberate pause has reverted in-place edits, resume
without re-running the original subtask sequence:

1. **Archive prior handoffs**:
   ```bash
   ...
   ```
2. **Collapse by file ownership**: create one resume agent per primary file/domain
   (definition file, hooks, docs, scripts/config).
3. **Dispatch in parallel**: each agent reads archived handoffs, applies outstanding
   changes for its owned files in one pass, and does not re-research.
4. **Verify and continue**: re-run drift check, then proceed to Phase 5a.
...
When dispatching subtasks that write `.md` definition files under `agents/`, `skills/`, or `workflows/`, append the classifier-outage addendum from routing config (`classifier_outage.dispatch_prompt_addendum`) to the dispatch prompt. The addendum text is config-resident so the workaround has one update site.
