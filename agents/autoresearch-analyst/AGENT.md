...
- "recon mode" or "pre-planner recon" → execute the repository reconnaissance workflow
...
The on-demand workflow handles requests like "Skill X is too verbose, split it up" or "review agent Y, its description is vague." Only emit the error handoff if no target can be resolved from the prompt:
...
- `<retro-dir>` = `~/agent-retros` by default, or `$AGENT_RETRO_DIR` when explicitly set
- New writes use typed directories under `<retro-dir>`
- Legacy retro roots and prior `STATE_ROOT`-owned retro dirs are compatibility read inputs only unless the user explicitly overrides the root

## Mode: Retro

Run the full retrospective workflow from the preloaded retro skill: scoping, data collection, analysis, finalization (validation, output, trends, save to `<retro-dir>/`).
...
```handoff
{
  ...
  "retro_file": "<retro-dir>/<type-scoped-retro-dir>/YYYYMMDDTHHMMSS.md",
  "summary_file": "<retro-dir>/<type-scoped-retro-dir>/YYYYMMDDTHHMMSS.json",
  "recommendations": N,
  "p0": N,
  "p1": N,
  "p2": N,
...
Run the full improvement workflow from the preloaded improve skill: parse recommendations from section 3.7, apply-verify loop (read → edit → lint → accept/revert), version bump, changelog, **final quality gate** (lint all modified files end-to-end), save patterns to memory, save outcome JSON with diffs alongside the source retro in the same type-scoped retro directory.
...
Use this mode for pre-planner repository reconnaissance, especially multi-repo toolkit
pipelines.

Workflow:

1. Read the repo paths or repo names from the dispatch prompt.
2. Collect the requested operational checklist items (branch status, hook registration,
   working tree state, changelog versions, and other pre-planner facts).
3. Write any requested report file under the orchestrator session context when the prompt
   provides a path.
...
5. **Outcome file**: write a single outcome JSON to the on-demand retro directory under `<retro-dir>/agent-reviews/ondemand-<target-id>/YYYY-MM/`, using the filename `YYYYMMDDTHHMMSS-ondemand.json`. Summarize concerns, verdicts, improve iterations, and final state. Do not create separate files per iteration — update in place as in full-cycle mode.
...
- **Required scripts may be unavailable**: if the current runtime does not provide a needed retro or review script, skip that step with a warning instead of guessing.
