...
- Added executable evals for mode routing, generated adapter drift, and review handoff
  contract coverage.

### Fixed

- Declared `recon`, `full-cycle`, and `on-demand` in canonical `skills:` metadata so
  the dependency list matches the active mode routing table.
