...
echo "$CMD" | grep -qE '\.claude|agent-toolkit|claude-toolkit|claude-code/hooks/|claude-code/agents/|claude-code/commands/|(^|/)(hooks|skills|rules)/|(^| )agents/|(^| )hooks/|(^| )CLAUDE\.md|(^| )settings\.json' || exit 0
...
#         agent-toolkit/ real-path equivalents, root-level hooks/, skills/, and rules/,
#         orchestrator/logs/,
...
# ------------------------------------------------------------------
PROTECTED='(\.claude/(settings\.json|hooks/|CLAUDE\.md|agents/|statusline-command\.sh)|agent-toolkit/|claude-toolkit/|claude-code/(agents|commands|hooks)/|(^|/)(hooks|skills|rules)/|\.orchestrator/(logs/|sessions/[0-9]{8}T[0-9]{6}/logs/|session\.id)|hookify[^/]*\.local\.md)'
